...
async function testAutoScroll() {
    ...
    try {
        ...
        console.log('  playheadLeft = ' + afterSeek.playheadLeft + 'px');
        console.log('  scrollLeft = ' + afterSeek.scrollLeft + 'px');
        console.log('  clientWidth = ' + afterSeek.clientWidth + 'px');

        // Calculate expected: playhead at 200s = 200 * 20 + 20 = 4020px
        const expectedPlayheadLeft = 200 * 20 + 20;
        if (Math.abs(afterSeek.playheadLeft - expectedPlayheadLeft) > 5) {
            console.log(`FAIL: Expected playhead at ~${expectedPlayheadLeft}px, got ${afterSeek.playheadLeft}px`);
            process.exit(1);
        }
        ...
        const visibleLeft = afterSeek.scrollLeft;
        const visibleRight = afterSeek.scrollLeft + afterSeek.clientWidth;
        const margin = afterSeek.clientWidth * 0.2;

        const isPlayheadVisible = (
            afterSeek.playheadLeft >= visibleLeft &&
            afterSeek.playheadLeft <= visibleRight
        );

        if (!isPlayheadVisible) {
            console.log('FAIL: Playhead is not in visible viewport');
            console.log(`  Visible range: ${visibleLeft}px - ${visibleRight}px`);
            console.log(`  Playhead at: ${afterSeek.playheadLeft}px`);
            process.exit(1);
        }
        console.log('Playhead is visible in viewport');

        // Test 6: Verify playhead is not too close to edges (has margin)
        const distanceFromLeft = afterSeek.playheadLeft - visibleLeft;
        const distanceFromRight = visibleRight - afterSeek.playheadLeft;
